...
### 3.4 `sweep_span_nm` 窄扫描 + `max_peak_wavelength_nm` 伪峰屏蔽（本次新增）

**窄扫流程**：区域两端的窗位置先用宽 span 测量（**锚点**——它们的波长在测之前是未知的，只能宽扫）；两个锚点连成直线预测每列的波长；其余每个位置把窄 span（如 1 nm）重定中心到预测波长再测。AUTO 采样下每条 trace 的点数随 span 缩小（1 nm vs 8 nm ≈ 8×），与 `coordinate_stride` 组合可达 ~40× 采样量削减。暗/亮参考帧仍用宽 span 测一次，归一化时内插到窄网格上。

**归一化地板**：亮参考低于自身峰值 5% 的波长样本不参与归一化——那里分母（亮参考−暗背景）≈ 0，会把漂移残差放大成伪峰（这就是此前 stride 扫描中个别点偏离直线的根因）。

**伪峰屏蔽**：本装置在 ~781.7 nm 有一个 SLM 调制不到的固定漏光峰（光落在有效区之外）。`max_peak_wavelength_nm≈781.5` 直接把寻峰范围截到它之下。
...
| Step 2 (wl_map) | `sweep_span_nm` 设 ~1 nm | 锚点+预测重定中心的窄扫（§3.4）；每条 trace 点数 ≈ ÷8 |
